...
import { createLinkRoleInUsers } from "@/api/role/link_role_in_users"

interface Props {
  refetchRules: () => void
}

export function LinkRuleUserModal({ refetchRules }: Props) {
  ...
  const { mutateAsync: linkRuleFarms } = useMutation({
    mutationFn: async (data: { ruleId: string; farmIds: string[] }) => {
      const dataFormated = {
        role_id: data.ruleId,
        users_ids: data.farmIds
      }
      await createLinkRoleInUsers(dataFormated)
      return data
    },
    onSuccess: () => {
      refetchRules()
  ...
  const filteredFarms = users?.filter(
    (user) =>
      user.name.toLowerCase().includes(searchFarms.toLowerCase()) ||
      user.email.toLowerCase().includes(searchFarms.toLowerCase()),
  )
      ...
      <DialogHeader>
        <DialogTitle className="flex items-center gap-2">
          <Link className="h-5 w-5" />
           Vincular Funções a usuários
        </DialogTitle>
        <DialogDescription>Arraste e solte para vincular rules com usuários ou use os botões de seleção</DialogDescription>
      </DialogHeader>
          ...
          >
            {selectedRule ? (
              ...
              <div className="text-center text-muted-foreground">
                ...
                <p className="text-sm">Arraste uma função aqui</p>
              </div>
            )}
          </div>
          ...
          >
            {selectedFarms.length > 0 ? (
              <div className="space-y-2">
                <p className="text-sm font-medium mb-2">Usuários Selecionadas:</p>
                {selectedFarms.map((farmId) => {
                  const farm = users?.find((f) => f.id === farmId)
                  return farm ? (
                    <div
                      key={farmId}
              ...
              <div className="text-center text-muted-foreground h-full flex items-center justify-center">
                <div>
                  ...
                  <p className="text-sm">Arraste usuário aqui</p>
                </div>
              </div>
            )}
          </div>
              ...
              >
                ...
                <CardContent>
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      <span className="font-medium">Nome:</span> {farm?.name}
                    </p>
                  </div>
                </CardContent>
              </Card>
